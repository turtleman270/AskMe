...
for rate in oldMountainViewRent{
    newMountainViewRent.append(rate * 1.2)
}
...
for (item,rate) in polarizingFoods{
    if rate == true{
        print(item + "yum!")
    }
    else{
...
for (area, code) in rapperPhoneNumbers{
        ...
        print(code.stringByReplacingOccurrencesOfString("415", withString: "510"))
