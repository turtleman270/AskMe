...
var word = "bologna"
var c = word.endIndex
word.removeAtIndex(c.predecessor())
...
func combineLastCharacters (nonsenseArray:[String]) -> String {
    var newArray : String
    newArray = ""
    for var item in nonsenseArray {
        let c = item.removeAtIndex(item.endIndex.predecessor())
        newArray += "\(c)"
    }
    return newArray
}
