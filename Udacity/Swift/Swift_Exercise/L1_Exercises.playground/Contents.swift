...
let yourSentence = "\(nounArray[3]) is not good \(nounArray[8])"
let yourSillySentence = "random array \(nounArray[index1])"
//: ### Exercise 2
...
var count0 = 0
for char in howManyCharacters.characters {
    count0+=1
}
...
if word.rangeOfString(word) != nil{
    print("exist")
}
//: ### Exercise 6
...
for (stringName,stringPic) in dictionary {
    print("\(stringName)")
    newestString = newestString.stringByReplacingOccurrencesOfString(stringName, withString: stringPic)
}
newestString
//: ### Exercise 8
...
//desired output = "$5.67"
var dollar = numOfPennies/100
var cents = numOfPennies%100

var output = "$\(dollar).\(cents)"
//: # Let or Var?
import UIKit
import Foundation
//: ### Exercise 9
